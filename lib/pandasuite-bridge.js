(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.PandaBridge = factory());
	...
	var hasOwnProperty$9 = uncurryThis$5({}.hasOwnProperty);
	...
	*/
	...
	function responseKey() {
	  ...
	  const unique = crypto && typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : `${Date.now()}-${responseCounter += 1}`;
	...
	function responseKeyOf(args) {
	  ...
	  return isString(last) && startsWith(last, PandaBridge.RESPONSE_EVENT_PREFIX) ? last : undefined;
	}
	...
	function settleFrom(key, listener, args) {
	  ...
	  Promise.resolve(returned).then(value => respond(key, okOutcome(value)), error => respond(key, failedOutcome(error)));
	}
	...
	PandaBridge.resolveResource = function resolveResource(id) {
	  const resources = PandaBridge.resources.filter(resource => resource.id === id && resource.path);
	...
	const replaceUrlsInStringWithDataUrls = async input => {
	  ...
	  const urlToReplacementPairs = await Promise.all(uniqueUrls.map(async url => {
	    const dataUrl = await urlToDataURL(url);
	    return [url, dataUrl || url];
	  }));
	  const replacements = {};
	  urlToReplacementPairs.forEach(_ref => {
	    let [k, v] = _ref;
	    replacements[k] = v;
	  });
	...
	PandaBridge.resolveTypes = async function resolveTypes(value) {
	  ...
	  if (isObject(value)) {
	    ...
	    return fromPairs(await Promise.all(map(value, async (v, k) => [k, await this.resolveTypes(v)])));
	  }
	  return value;
	};
	...
	/* Binder class */
	const Binder = {};
	Binder.resolveShortTags = createAsyncBridgeMethod(PandaBridge.RESOLVE_SHORT_TAGS);
	Binder.resolveDeepShortTags = createAsyncBridgeMethod(PandaBridge.RESOLVE_DEEP_SHORT_TAGS);
